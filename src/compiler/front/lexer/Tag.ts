/**
 * @author: Peixe
 * @description: A Tag is the predecessor of Tokens. They are used to identify sequences in the tokenizer/lexer process.
 * @example: Tag.
 */
...
class Tag {
    ...
    static Identifier = this.create(Lexer.matchIdentifier, "Identifier")
    static String = this.create(Lexer.matchString, "String")
    static Integer = this.create(Lexer.matchNumber, "Integer")
    static Float = this.create(Lexer.matchNumber, "Float")

    constructor(content: string | string[] | Function, description: string) {
        this.content = content
        this.symbol = Symbol.for(description)
    }

    static create(content: string | string[] | Function, description: string): Tag {
